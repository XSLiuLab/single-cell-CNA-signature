...
SP_PCAWG <- sigprofiler_import("./ACE_NEW/call_signature/new_ploidy_pcawgSignature/ploidy_pcawgSExtract/", order_by_expo = TRUE, type = "all")

saveRDS(SP_PCAWG, file = "./ACE_NEW/call_signature/new_ploidy_pcawgSignature/result/pcawg_ploidy_solutions_sp.rds")
...
pcawg_sigs <- SP_PCAWG$solution_list$S5
apply(pcawg_sigs$Exposure, 1, mean)
sig_names(pcawg_sigs)
colnames(pcawg_sigs$Signature) <- colnames(pcawg_sigs$Signature.norm) <- rownames(pcawg_sigs$Exposure) <- rownames(pcawg_sigs$Exposure.norm) <- pcawg_sigs$Stats$signatures$Signatures <- paste0("pcawgSignature", 1:5)
sig_names(pcawg_sigs)


saveRDS(pcawg_sigs, file = "./ACE_NEW/call_signature/new_ploidy_pcawgSignature/result/new_pcawg_ploidy_sigs_signature.rds")
pcawg_sigs <- readRDS("./ACE_NEW/call_signature/new_ploidy_pcawgSignature/result/new_pcawg_ploidy_sigs_signature.rds")


pcawg_act <- list(
  absolute = get_sig_exposure(SP_PCAWG$solution_list$S5, type = "absolute"),
  relative = get_sig_exposure(SP_PCAWG$solution_list$S5, type = "relative", rel_threshold = 0),
  similarity = pcawg_sigs$Stats$samples[, .(Samples, `Cosine Similarity`)]
)

colnames(pcawg_act$absolute)[-1] <- colnames(pcawg_act$relative)[-1] <- paste0("pcawgSignature", 1:5)
...
df_abs <- get_sig_exposure(pcawg_sigs) %>%
  tidyr::pivot_longer(cols = starts_with("pcawgSignature"), names_to = "sig", values_to = "activity") %>%
  dplyr::mutate(
    activity = log10(activity + 1),
    sig = factor(sig, levels = paste0("pcawgSignature", 1:7))
  )
...
get_sample_from_sig <- function(dt, sig) {
  res <- head(dt[order(dt[[sig]], dt[[paste0("ABS_", sig)]], decreasing = TRUE)], 3L)
  res
}



pcawg_cn_obj <- readRDS("./ACE_NEW/pcawg_low/result/pcawg_cn_1Mb_ACE.rds")
...
colnames(abs_activity) <- c("sample", paste0("ABS_pcawgSignature", 1:7))
act <- merge(
...
dir.create("./ACE_NEW/call_signature/ploidy_pcawgSignature/result/enrich_samples/", showWarnings = FALSE)
for (i in paste0("pcawgSignature", 1:7)) {
  ...
  print(p)
  ggplot2::ggsave(file.path("./ACE_NEW/call_signature/ploidy_pcawgSignature/result/enrich_samples/", paste0(i, ".pdf")),
